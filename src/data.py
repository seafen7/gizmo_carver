                    ...
                    if isinstance(self.grid, radmc3dOctree):
                        ...
                        if hdr[1] == 8:
                            ...
                            raise TypeError(
                                ...
                                + 'floats or 8 byte doubles. The precision in the file header is '
                                + ("%d" % hdr[1]))
                        self.gasmag = np.reshape(self.gasmag, [self.nLeaf, 3], order='f')
                        ...
                        if hdr[1] == 8:
                            ...
                            raise TypeError(
                                ...
                                + 'floats or 8 byte doubles. The precision in the file header is '
                                + ("%d" % hdr[1]))
                        self.gasmag = np.reshape(self.gasmag, [self.grid.nz, self.grid.ny, self.grid.nx, 3])
                ...
                with open(fname, 'r') as rfile:
                    ...
                    if octree:
                        ...
                        if (self.grid.nx * self.grid.ny * self.grid.nz) != hdr[1]:
                            ...
                            raise RuntimeError(msg)
                        data = np.fromfile(rfile, count=-1, sep=" ", dtype=np.float64)
                        self.gasmag = np.reshape(data, [self.grid.nz, self.grid.ny, self.grid.nx, 3])
                        self.gasmag = np.swapaxes(self.gasmag, 0, 2)
