...
class radmc3dData(object):
    ...
    def __init__(self, grid=None):
        ...
        self.gasmag = np.zeros(0, dtype=np.float64)
    ...
    def _findDataFile(self, basename):
        ...
        if hits > 1:
            print(
                "Warning: Multiple files basename "
                + basename
                + " found. Taking "
                + filename
            )
            ...
            if binary:
                if octree:
                    if len(data.shape) == 1:
                        ...
                        hdr = np.array(
                            [1, 8, data.shape[0], data.shape[1]], dtype=np.int64
                        )
                    else:
                        raise ValueError(
                            "Incorrect shape. Data stored in an Octree-type grid should have 1 or 2 "
                            + "dimensions, with the second dimension being the dust species. "
                            + " The data to be written has a dimension of "
                            + ("%d" % len(data.shape))
                    ...
                    if len(data.shape) == 3:
                        hdr = np.array(
                            [1, 8, self.grid.nx * self.grid.ny * self.grid.nz],
                            dtype=np.int64,
                        )
                        hdr.tofile(wfile)
                    ...
                    if len(data.shape) == 4:
                        ...
                        raise ValueError(
                            "Incorrect shape. Data stored in a regular grid should have 3 or 4 dimensions"
                ...
                if octree:
                    if len(data.shape) == 1:
                        ...
                        hdr = np.array(
                            [1, data.shape[0], data.shape[1]], dtype=np.int64
                        )
                    else:
                        raise ValueError(
                            "Incorrect shape. Data stored in an Octree-type grid should have 1 or 2 "
                    ...
                    if len(data.shape) == 3:
                        hdr = np.array(
                            [1, self.grid.nx * self.grid.ny * self.grid.nz], dtype=int
                        )
                        hdr.tofile(wfile, sep=" ", format="%d\n")
                        # Now we need to flatten the dust density array since the Ndarray.tofile function writes the
                        # array always in C-order while we need Fortran-order to be written
                ...
                if octree:
                    if hdr[2] != self.grid.nLeaf:
                        ...
                        msg = (
                            "Number of cells in "
                            + fname
                            + " is different from that in amr_grid.inp"
                            + " nr cells in "
                            + fname
                            + " : "
                            + ("%d" % hdr[2])
                    ...
                    if hdr[1] == 8:
                        ...
                        msg = (
                            "Unknown datatype/precision in "
                            + fname
                            + ". RADMC-3D binary files store 4 byte "
                    ...
                    if ndim == 3:
                        if data.shape[0] == hdr[2]:
                            ...
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
                        ...
                        if data.shape[0] == hdr[2] * hdr[3]:
                            data = np.reshape(
                                data, [self.grid.nLeaf, hdr[3]], order="f"
                            )
                        else:
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
                    ...
                    if ndim == 3:
                        if data.shape[0] == hdr[2]:
                            data = np.reshape(
                                data, [1, self.grid.nz, self.grid.ny, self.grid.nx]
                            )
                        else:
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
                        ...
                        if data.shape[0] == hdr[2] * hdr[3]:
                            data = np.reshape(
                                data, [hdr[3], self.grid.nz, self.grid.ny, self.grid.nx]
                            )
                        else:
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
                ...
                if octree:
                    ...
                    if ndim == 3:
                        ...
                        if data.shape[0] == hdr[1] * hdr[2]:
                            data = np.reshape(
                                data, [self.grid.nLeaf, hdr[2]], order="f"
                            )
                        else:
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
                    ...
                    if ndim == 3:
                        ...
                        if data.shape[0] == hdr[1] * hdr[2]:
                            data = np.reshape(
                                data, [hdr[2], self.grid.nz, self.grid.ny, self.grid.nx]
                            )
                        else:
                            msg = (
                                "Internal inconsistency in data file, number of cell entries is different from "
    ...
    def getTauOneDust(self, idust=0, axis="", kappa=0.0):
        ...
        if do_taux:
            taux = np.zeros(
                [self.grid.nx, self.grid.ny, self.grid.nz], dtype=np.float64
            )
            ...
            for ix in range(1, self.grid.nx):
                taux[ix, :, :] = (
                    taux[ix - 1, :, :]
                    + self.rhodust[ix, :, :, idust] * kappa * diff_x[ix]
                )
        else:
            taux = [-1.0]
        ...
        if do_tauy:
            ...
            for iy in range(1, self.grid.ny):
                tauy[:, iy, :] = (
                    tauy[:, iy - 1, :]
                    + self.rhodust[:, iy, :, idust] * kappa * diff_y[iy] * dum_x
                )
        else:
            tauy = [-1.0]
    ...
    def getTau(self, idust=None, axis="xy", wav=0.0, kappa=None, old=False):
        ...
        if max(idust) > ndust:
            raise ValueError(
                " There are less number of dust species than some of the indices in idust"
            )
        ...
        if kappa is not None:
            ...
            if len(kappa) != len(idust):
                raise ValueError(
                    " The number of kappa values should be identical to the number of specified "
                    + "dust species "
                )
        else:
            if not old:
                ...
                if len(dummy_ext) <= max(idust):
                    raise ValueError(
                        "There are less dust species specified in dustopac.inp than "
            ...
            if kappa is None:
                ...
                if not opac.ext:
                    ...
                    if opac.wav[0][-1] > opac.wav[0][0]:
                        kabs = 10.0 ** np.interp(
                            np.log10(np.array(wav)),
                            np.log10(opac.wav[0]),
                            np.log10(opac.kabs[0]),
                        )
                    else:
                        kabs = 10.0 ** np.interp(
                            np.log10(np.array(wav)),
                            np.log10(opac.wav[0][::-1]),
    ...
    def readDustDens(self, fname=None, old=False):
        ...
        #
            ...
            if os.path.isfile(fname):
                ...
                self.rhodust = np.zeros(
                    [hdr[1], hdr[2] * 2, 1, hdr[0]], dtype=np.float64
                )
        ...
        if not old:
            ...
            if os.path.isfile(fname):
                ...
                for idust in range(hdr[0]):
                    ...
                    # temperature arrays
                    data = data[1:]
                    self.dusttemp[:, : hdr[2], 0, idust] = np.reshape(
                        data[:ncell], [hdr[1], hdr[2]]
                    )
                self.dusttemp[:, hdr[2] :, 0, :] = self.dusttemp[:, : hdr[2], 0, :][
                    :, ::-1, :
                ]
            ...
            if binary:
                ...
                with open(fname, "r") as rfile:
                    if isinstance(self.grid, radmc3dOctree):
                        ...
                        self.gasvel = np.reshape(
                            self.gasvel, [self.nLeaf, 3], order="f"
                        )
                        ...
                        self.gasvel = np.reshape(
                            self.gasvel, [self.grid.nz, self.grid.ny, self.grid.nx, 3]
                        )
                ...
                with open(fname, "r") as rfile:
                    ...
                    if octree:
                        ...
                        data = np.fromfile(rfile, count=-1, sep=" ", dtype=np.float64)
                        self.gasvel = np.reshape(
                            data, [self.grid.nz, self.grid.ny, self.grid.nx, 3]
                        )
                        self.gasvel = np.swapaxes(self.gasvel, 0, 2)

    def readGasMag(self, fname=None):
        """Reads the magnetic field, identical format to gas velocity.

        Parameters
        -----------

        fname : str, optional
                Name of the file that contains the gas magnetic field
                If omitted 'gas_magnetic_field.inp' or 'gas_magnetic_field.binp' is used.
        """
        assert self.grid is not None, "radmc3dData object MUST have a .grid subobject."
        ...
        if fname is None:
            fname = self._findDataFile("gas_magnetic_field")
        if os.path.isfile(fname):
            binary = self._isBinary(fname)
            if binary:
                print("Reading " + fname)
                ...
                with open(fname, "r") as rfile:
                    if isinstance(self.grid, radmc3dOctree):
                        ...
                        if hdr[1] == 8:
                            self.gasmag = np.fromfile(rfile, count=-1, dtype=np.float64)
                        elif hdr[1] == 4:
                            self.gasmag = np.fromfile(rfile, count=-1, dtype=np.float32)
                        else:
                            raise TypeError(
                                "Unknown datatype/precision in "
                                + fname
                        ...
                        self.gasmag = np.reshape(
                            self.gasmag, [self.nLeaf, 3], order="f"
                        )
                        ...
                        if hdr[1] == 8:
                            ...
                            self.gasmag = np.fromfile(rfile, count=-1, dtype=float)
                        else:
                            raise TypeError(
                                "Unknown datatype/precision in "
                                + fname
                        ...
                        self.gasmag = np.reshape(
                            self.gasmag, [self.grid.nz, self.grid.ny, self.grid.nx, 3]
                        )
                        self.gasmag = np.swapaxes(self.gasmag, 0, 2)
            else:
                print("Reading " + fname)
                with open(fname, "r") as rfile:
                    # Two element header 1 - iformat, 2 - Nr of cells
                    ...
                    if octree:
                        if self.grid.nLeaf != hdr[1]:
                            ...
                            raise RuntimeError(msg)
                        data = np.fromfile(rfile, count=-1, sep=" ", dtype=np.float64)
                        self.gasmag = np.reshape(data, [hdr[1], 3])
                    else:
                        if (self.grid.nx * self.grid.ny * self.grid.nz) != hdr[1]:
                            msg = (
                                "Number of grid cells in "
    ...
    def writeDustDens(self, fname="", binary=True, old=False):
        ...
        if not old:
            ...
            if octree:
                self._scalarfieldWriter(
                    data=self.grid.convArrTree2Leaf(self.rhodust),
                    fname=fname,
                    binary=binary,
                )
            else:
        ...
        #
            ...
            if self.rhodust.shape[2] > 1:
                raise ValueError(
                    "Wrong dimensions for dust density. RADMC (the predecessor code to RADMC-3D is "
    ...
    def writeDustTemp(self, fname="", binary=True):
        ...
        if octree:
            self._scalarfieldWriter(
                data=self.grid.convArrTree2Leaf(self.dusttemp),
                fname=fname,
                binary=binary,
            )
        else:
    ...
    def writeGasDens(self, fname=None, ispec="", binary=True):
        ...
        if fname is None:
            if ispec == "":
                raise ValueError(
                    "Unknown ispec. Without knowing the name of the gas species the gas density "
        ...
        if octree:
            self._scalarfieldWriter(
                data=self.grid.convArrTree2Leaf(self.ndens_mol),
                fname=fname,
                binary=binary,
            )
        else:
    ...
    def writeGasTemp(self, fname="", binary=True):
        ...
        if octree:
            self._scalarfieldWriter(
                data=self.grid.convArrTree2Leaf(self.gastemp),
                fname=fname,
                binary=binary,
            )
        else:
        ...
        if binary:
            ...
            with open(fname, "w") as wfile:
                if octree:
                    ...
                    hdr = np.array(
                        [1, 8, self.grid.nx * self.grid.ny * self.grid.nz], dtype=int
                    )
                    hdr.tofile(wfile)
            ...
            if octree:
                ...
                if self.gasvel.shape[0] == self.grid.nLeaf:
                    ...
                    try:
                        np.savetxt(
                            fname, dummy, fmt="%.9e %.9e %.9e", header=hdr, comments=""
                        )
                    except Exception as e:
                        print(e)
                        ...
                        for iy in range(self.grid.ny):
                            for ix in range(self.grid.nx):
                                wfile.write(
                                    "%9e %9e %9e\n"
                                    % (
                                        self.gasvel[ix, iy, iz, 0],
                                        self.gasvel[ix, iy, iz, 1],
    ...
    def writeVTurb(self, fname="", binary=True):
        ...
        if octree:
            self._scalarfieldWriter(
                data=self.grid.convArrTree2Leaf(self.vturb), fname=fname, binary=binary
            )
        else:
    ...
    def writeVTK(
        ...
        if (ddens is True) | (dtemp is True):
            if idust is None:
                msg = (
                    "Unknown dust species. Dust density or temperature is to be written but  "
