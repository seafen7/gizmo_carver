...
if os.path.isfile('%s.out' % inputFileNameBase) is True:
	...
	with open('%s.out' % inputFileNameBase) as image:
		...
		for line in image:
			...
			#
				...
				intensity_ergSter = numpy.zeros((numberChannels, numberPixels_y, numberPixels_x))
			...
			#
			# get the wavelengths
			elif (counter >= 5) and (counter < (5 + numberChannels)) and (len(line.split()) > 0):
			...
			# get the intensities
				...
				intensity_ergSter[numpy.int(counteri/numberPixels_x/numberPixels_y), numpy.int(counteri/numberPixels_x)%numberPixels_y ,counteri%numberPixels_x] = line
				counteri += 1
	...
	if getBandwidthFromFile == 0:
		...
		if spectral_axis_flag == 0:
			print("")
			print("	Calculated a velocity bandwidth of %.10f m/s from input file" % delta_v_ms)

		elif spectral_axis_flag == 1:
			print("")
			print("	Calculated a frequency bandwidth of %.10f Hz from input file" % delta_nu_hz)
	...
	for intensity_unit_flag in intensity_unit:
		...
		if intensity_unit_flag == 0:
			...
			hdu = pyfits.PrimaryHDU(numpy.array(intensity_ergSter * erg2jy * ster2pixel))

		elif intensity_unit_flag == 3:
			hdu = pyfits.PrimaryHDU(numpy.array(intensity_ergSter.T * wavelength_cm**2).T / (2. * kB))
		...
		hdu.header['equinox'] = 2000.0						# epoch keyword is deprecated, equinox should be used instead
		hdu.header['restfrq'] = restfrequency_hz			  # physical unit must be Hertz!

		#
		# first axis (right-ascension)

		hdu.header['ctype1'] = 'RA---SIN'					 # type of the coodinate system; SIN = Slant orthographic
		hdu.header['cdelt1'] = -pixelSize_x_degree			# x coordinate increment; minus sign, since RA axis points from right to left
		hdu.header['crval1'] = ra_refpos_degree			   # world coordinate at reference point
		hdu.header['crpix1'] = (numberPixels_x + 1.0) / 2.0   # reference pixel x coordinate
		hdu.header['cunit1'] = 'degree'

		#
		# second axis (declination)

		hdu.header['ctype2'] = 'DEC--SIN'					 # type of the coodinate system; SIN = Slant orthographic
		...
		hdu.header['crpix2'] = (numberPixels_y + 1.0) / 2.0   # reference pixel y coordinate
		...
		if spectral_axis_flag == 0:
			...
			hdu.header['crval3'] = restfrequency_hz * (1 - velocityCenter_kms * 1.0e3 / c_ms)
		...
		with warnings.catch_warnings():
			...
			pyfits.writeto('%s_%s' % (outputFileNameBase, fileEnding), numpy.float32(hdu.data), hdu.header, clobber=True)
