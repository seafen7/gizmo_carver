...
)  # same as carver_CarveOut
from yt.extensions.astro_analysis.radmc3d_export.RadMC3DInterface import (
    RadMC3DLayer,
    RadMC3DSource,
)
...
class RadMC3DWriter_Gizmo:
    ...
    def write_amr_grid(self, filename):
        ...
        dims = (
            self.domain_dimensions
        )  # if force_unigrid, this should have been overwritten to the right value
        ...
        RE_cgs = RE.in_units(
            "cm"
        ).d  # also prevents passing by pointer (self.domain... etc. needs to be used again)
        ...
        for layer in self.layers[1:]:  # [1:] skips entry 0, the base layer
            ...
            dds = (layer.RightEdge - layer.LeftEdge) / (
                layer.ActiveDimensions
            )  # cell size
            if p == 0:  # parent is the base layer,
                ...
                # Beginning index
                ind = (layer.LeftEdge - LE) / (
                    2.0 * dds
                ) + 1  # LE wasn't shifted so 0,0,0 is center. OK!
                ...
                ind = (layer.LeftEdge - parent_LE) / (
                    2.0 * dds
                ) + 1  # #index in parent grid cells; periodic or not, they both should be correct; difference is good here
            # print(str(np.array(ind)+0.5) + " " + str([int(x) for x in np.array(ind)+0.5]))
            ix = int(
                ind[0] + 0.5
            )  # beginning index in terms of parent grid cells (b/c of the 2*dds)
            iy = int(ind[1] + 0.5)
            iz = int(ind[2] + 0.5)
            ...
            # iz = math.ceil(ind[2])
            nx, ny, nz = (
                layer.ActiveDimensions / 2
            )  # number of cells (/2 to measure in dimensions of parent cell size)
            # print(nx,ny,nz)
            s = "{}    {}    {}    {}    {}    {}    {} \n"
            ...
            s = s.format(
                p, ix, iy, iz, int(nx), int(ny), int(nz)
            )  # RAD complains if anything is not int()
    ...
    def _write_layer_data_to_file(self, fhandle, field, level, LE, dim):
        ...
        if isinstance(field, list):
            ...
            # Shift by COM
            meanvx = np.sum(data_x * cg[("PartType0", "Density")]) / np.sum(
                cg[("PartType0", "Density")]
            )
            meanvy = np.sum(data_y * cg[("PartType0", "Density")]) / np.sum(
                cg[("PartType0", "Density")]
            )
            meanvz = np.sum(data_z * cg[("PartType0", "Density")]) / np.sum(
                cg[("PartType0", "Density")]
            )
            print(
                "Shift velocities by mean ",
                np.mean(data_x),
                np.mean(data_y),
                np.mean(data_z),
            )
    ...
    def writeMolLineFile(self, filename, vmax, nwav, restfreq):
        """
        This method writes the set of wavelengths radmc needs for a molecular line calculation.
        Output is linearly spaced in velocity [-vmax, vmax] about the line rest wavelength
        ----------
        nwav : int
        ...
        vels = np.linspace(-vmax, vmax, nwav) * 1e5  # [cm/s]
        restwav = const.c.cgs.value / restfreq * 1e4  # Convert frequency to micron
        wav = restwav / (1.0 - vels / const.c.cgs.value)
